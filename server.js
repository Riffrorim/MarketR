...
app.post("/productData", upload.array('photos', 5), async (req, res) => {
  ...
  try {
    ...
    const [userResult] = await con.promise().query(
    ...
    await con.promise().query(
      `INSERT INTO products 
       (name, description, price, photo, user_id, creator_name) 
       VALUES (?, ?, ?, ?, ?, ?)`,
      [nameProduct, description, price, photoUrls.join(','), user_id, creatorName]
    );
...
app.get("/getUserProducts", (req, res) => {
    ...
    const query = "SELECT * FROM products WHERE user_id = ? AND sold_at IS NULL";
    con.query(query, [userId], (err, results) => {
        if (err) {
            console.error("Database error:", err);
            return res.status(500).json({ error: "Ошибка базы данных" });
...
app.post("/deleteProduct", (req, res) => {
    ...
    con.query(checkQuery, [product_id, user_id], (err, result) => {
        if (err) {
            console.error("Database error:", err);
            return res.status(500).json({ error: "Ошибка базы данных" });
        }
        ...
        if (result.length === 0) {
            return res.status(403).json({ error: "Вы не можете удалить этот товар" });
        }

        // Удаляем товар
        const deleteQuery = "DELETE FROM products WHERE id = ?";
        con.query(deleteQuery, [product_id], (err, deleteResult) => {
            if (err) {
                console.error("Database error:", err);
                return res.status(500).json({ error: "Ошибка базы данных" });
            }
