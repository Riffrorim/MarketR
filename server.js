...
const con = mysql.createConnection({
    host: "localhost",
    user: "riffrorim",
    password: "riffrorim31",
    database: "market_base",
});
...
app.post("/productData", upload.array('photos', 5), (req, res) => {
  const { nameProduct, description, price, user_id } = req.body;
  
  if (!nameProduct || !description || !price || !user_id || !req.files || req.files.length === 0) {
    return res.status(400).json({ error: "Все поля обязательны для заполнения" });
  }
  const getUserQuery = "SELECT name FROM users WHERE id = ?";
  con.query(getUserQuery, [user_id], (err, userResult) => {
    if (err) {
      console.error("Database error:", err);
      return res.status(500).json({ error: "Ошибка базы данных при получении имени пользователя" });
    }
    ...
    const photoPaths = req.files.map(file => `/uploads/${file.filename}`).join(',');
